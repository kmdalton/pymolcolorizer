...
class attributePainter:
    ...
    def __init__(self, csvFN=None, **kw): -> None
        ...
        for i, line in enumerate(open(csvFN).readlines(), 1):
            if line[0] != "#":
                ...
                try:
                    ...
                    print(
                        "Line number {}  ({}) was rejected -- could not format".format(
                            i, line
                        )
                    )
        ranges = {
            chainID: (min(chain.values()), max(chain.values()))
            for chainID, chain in self.chains.items()
        }
        self.norms = {chainID: Normalize(r[0], r[1]) for chainID, r in ranges.items()}
        self.sm = {
            chainID: ScalarMappable(norm, self.cmap)
            for chainID, norm in self.norms.items()
        }
        self.norms["global"] = Normalize(
            min([i[0] for i in ranges.values()]), max([i[1] for i in ranges.values()])
        )
    ...
    def modifyBfactors(self, chainID, **kw):
        ...
        for resnum, value in self.chains[chainID].items():
            cmd.alter(
                "chain {} and resi {}".format(chainID, resnum), "b={}".format(value)
            )
